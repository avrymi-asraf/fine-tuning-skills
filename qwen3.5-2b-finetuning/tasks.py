...
# ///
"""
Invoke tasks for Qwen 3.5 2B fine-tuning.
Primary path: local training. Cloud (Vertex AI) is optional.

Usage:
    uv run tasks.py --list

    # Local training (default)
    uv run tasks.py prepare-data
    uv run python train_hf_peft.py --dataset_path data/formatted_dataset

    # Optional: one command for the full cloud flow
    uv run tasks.py run --project-id my-project --bucket-name my-qwen-bucket

    # Optional helpers
    uv run tasks.py submit --project-id my-project --bucket-name my-qwen-bucket
...
IMAGE_NAME = os.getenv("IMAGE_NAME", "qwen3.5-train")
IMAGE_TAG = os.getenv("IMAGE_TAG", "v1.0.0")
BUCKET_NAME = os.getenv("BUCKET_NAME", "")
DATASET_PATH = os.getenv("DATASET_PATH", "data/formatted_dataset")
DATASET_GCS_PREFIX = os.getenv("DATASET_GCS_PREFIX", "datasets/qwen3.5-2b-v1")
...
def submit(c, project_id=PROJECT_ID, region=REGION, display_name="qwen3.5-2b-train"):
